package com.wartec.wartecmod.compat.client;

import net.minecraft.item.ItemStack;
import net.minecraftforge.client.IItemRenderer;
import org.lwjgl.opengl.GL11;

public final class ItemRenderTacticalAircraft implements IItemRenderer {
    private final RenderTacticalAircraft renderer;
    private final int variant;

    ItemRenderTacticalAircraft(RenderTacticalAircraft renderer, int variant) {
        this.renderer = renderer;
        this.variant = variant;
    }

    @Override public boolean handleRenderType(ItemStack item, ItemRenderType type) {
        return true;
    }
    @Override public boolean shouldUseRenderHelper(ItemRenderType type, ItemStack item,
            ItemRendererHelper helper) { return true; }

    @Override
    public void renderItem(ItemRenderType type, ItemStack item, Object... data) {
        GL11.glPushMatrix();
        if (type == ItemRenderType.INVENTORY) {
            GL11.glTranslatef(0.0F, -0.04F, 0.0F);
            GL11.glScalef(1.10F, 1.10F, 1.10F);
        }
        renderer.renderInventory(variant);
        GL11.glPopMatrix();
    }
}
